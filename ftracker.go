...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	avgSpeedMiS := meanSpeed(action, duration) * kmhInMsec // скорость в метрах в секунду
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeedMiS, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	avgSwimSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (avgSwimSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
